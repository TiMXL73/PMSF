    ...
    public function generated_exclude_list($type)
    {
        ...
        if ($type === 'pokemonlist') {
            ...
            if (!$noQuestsARTaskToggle) {
                $pokestops = array_unique(array_merge($pokestops, $db->query("SELECT distinct alternative_quest_pokemon_id AS reward_pokemon_id FROM pokestop WHERE alternative_quest_pokemon_id > 0 AND DATE(FROM_UNIXTIME(alternative_quest_timestamp)) = '" . $curdate->format('Y-m-d') . "' AND alternative_quest_reward_type = 7 ORDER BY reward_pokemon_id;")->fetchAll(\PDO::FETCH_ASSOC)),SORT_REGULAR);
            }
            $data = array();
            foreach ($pokestops as $pokestop) {
            ...
            if (!$noQuestsARTaskToggle) {
                $pokestops = array_unique(array_merge($pokestops, $db->query("SELECT distinct json_extract(json_extract(`alternative_quest_rewards`,'$[*].info.pokemon_id'),'$[0]') AS reward_pokemon_id FROM pokestop WHERE alternative_quest_reward_type = 12;")->fetchAll(\PDO::FETCH_ASSOC)),SORT_REGULAR);
            }
            $data = array();
            foreach ($pokestops as $pokestop) {
                $data[] = $pokestop['reward_pokemon_id'];
            ...
            if (!$noQuestsARTaskToggle) {
                $pokestops = array_unique(array_merge($pokestops, $db->query("SELECT distinct json_extract(json_extract(`alternative_quest_rewards`,'$[*].info.pokemon_id'),'$[0]') AS reward_pokemon_id FROM pokestop WHERE alternative_quest_reward_type = 4;")->fetchAll(\PDO::FETCH_ASSOC)),SORT_REGULAR);
            }
            $data = array();
            foreach ($pokestops as $pokestop) {
                $data[] = $pokestop['reward_pokemon_id'];
            ...
            if (!$noQuestsARTaskToggle) {
                $pokestops = array_unique(array_merge($pokestops, $db->query("SELECT distinct alternative_quest_item_id AS reward_item_id FROM pokestop WHERE alternative_quest_item_id > 0 AND DATE(FROM_UNIXTIME(alternative_quest_timestamp)) = '" . $curdate->format('Y-m-d') . "' ORDER BY reward_item_id;")->fetchAll(\PDO::FETCH_ASSOC)),SORT_REGULAR);
            }
            $data = array();
            foreach ($pokestops as $pokestop) {
